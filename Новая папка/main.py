...
def film_generator(generated_text:str) -> str:
...
def generate_recomendation(mood):
    prompt = (f"Посоветуй только один популярный фильм для человека у которого {mood} настроение. Напиши только название, без описания и комментариевб если жвер не указаон выбери сам подходящий ")
...
def submit():
    ...
    if request.method == "POST":
        ...
        if label == "POSITIVE":
            ...
            mood = "нейтральное"
        ai_text= film_generator(generate_recomendation(mood))
